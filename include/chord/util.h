...
void print_chordID(FILE *out, chordID *id) DLL_PUBLIC;
...
void v6_addr_copy(in6_addr *dest, const in6_addr *src) DLL_PUBLIC;
void v6_addr_set(in6_addr *dest, const uchar *src) DLL_PUBLIC;
char *buf_to_str(const uchar *buf, int len) DLL_PUBLIC;
char *buf_to_hex(const uchar *buf, int len);
void get_data_id(chordID *id, const uchar *buf, int n) DLL_PUBLIC;
void get_address_id(chordID *id, in6_addr *addr, ushort port) DLL_PUBLIC;
void to_v6addr(ulong v4addr, in6_addr *v6addr);
ulong to_v4addr(const in6_addr *v6addr) DLL_PUBLIC;
char *v6addr_to_str(const in6_addr *v6addr) DLL_PUBLIC;
